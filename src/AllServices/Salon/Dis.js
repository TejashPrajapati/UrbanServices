...
const Dis = ({ navigation, params }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `http://${IPPORT}:4000/api/disinfection`
      );
  ...
  const bookingData = async (item) => {
    try {
      ...
      showMessage({
        message: "Success",
        description: "Services Added To Bokking",
        type: "success",
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.white }}>
      ...
      <ImageBackground
        style={{ flex: 0.7 ,  height:450 , width: 430 }}
        source={require("../database/images/products/disinfections.jpg")}
      >
